...
class BookingController extends Controller{
    ...
    public function __construct(){
        ...
        $this->slug = '/booking-history';
        $this->title = 'Booking History';
        $this->table = 'users_booked_property';
    }
    ...
    public function index(Request $request){
        ...
        if($start_date){
            array_push($where, array('users_booked_property.created_at', '>=', $start_date));
            $serach_data['start_date'] = $start_date;
        }

        if($end_date){
            array_push($where, array('users_booked_property.created_at', '<=', $end_date));
        ...
        //posted_by_id
        $data['rows'] = $this->CommonModel->get_all($table = "users_booked_property", $select = array('users_booked_property.*', 'users.name', 'users.phone', 'property.title', 'property.address',  'property.property_for', 'property.posted_by', 'p_users.name as posted_by_name'), $where, $join = array(), $left = array(array('users', 'users.id', '=', 'users_booked_property.user_id'), array('property', 'property.id', '=', 'users_booked_property.property_id'), array('users as p_users', 'p_users.id', '=', 'property.posted_by_id')), $right = array(), $order = array(array('users_booked_property.id' => 'DESC')), $group = "", $limit = array(), $raw = "", $paging = "20");  
        //dd($data['rows']);   
        return view('admin.pages.booking.view', $data);
    }
