...
class DashboardController extends Controller {
    ...
    public function get_admin_dashboard() {
        ...
        //Property Count
        $data['property'] = DB::select("SELECT COUNT(id) AS total FROM property WHERE is_deleted = '0'");
        $data['user'] = DB::select("SELECT COUNT(id) AS total FROM users WHERE is_deleted = '0'");
        $data['aproved_property'] = DB::select("SELECT COUNT(id) AS total FROM property WHERE is_admin_aproved = '1'");
        $data['non_aproved_property'] = DB::select("SELECT COUNT(id) AS total FROM property WHERE is_admin_aproved = '0' AND is_deleted = '0'");
